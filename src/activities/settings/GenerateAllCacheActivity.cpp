...
void GenerateAllCacheActivity::render(RenderLock&&) {
  ...
  if (state == CONFIRMING) {
    drawCentered(centerY - 20, tr(STR_GENERATE_CACHE));
    drawCentered(centerY + 10, tr(STR_GENERATE_CACHE_NOTE));
  ...
  if (state == GENERATING) {
    drawCentered(centerY, tr(STR_GENERATING_ALL_CACHE));
    renderer.displayBuffer();
    return;
  }

  if (state == SUCCESS) {
    drawCentered(centerY - 20, tr(STR_CACHE_GENERATED), EpdFontFamily::BOLD);
